...
export interface RouteInfo {
  ...
  params: Record<string, string>
  /**
   * The URL's query, parsed: `/darkroom?preset=riso` → `{ preset: 'riso' }`.
   *
   * `id`, `path` and `params` are the COMMITTED route; `query` is the address
   * bar RIGHT NOW. The asymmetry is deliberate — `setSearchParam` writes the
   * query without re-resolving the route (so an overlay keeps its page, its
   * scroll and its canvases), and a value copied in when the route resolved
   * would be wrong from the first shallow write. Every read hits the live URL,
   * so it cannot go stale; `useRoute()` re-renders when it changes.
   *
   * `{}` on the server and during hydration — the query is client state, never
   * part of the SSR'd HTML, the same contract as `useSearchParam` returning
   * null there. Repeated keys keep the first value. Frozen: don't write to it.
   */
