...
export function PageScope({ entry, phase, hidden, registerEl }: PageScopeProps) {
  ...
  // Keyed on the ENTRY alone, deliberately: `route.query` is a live getter, so
  // this object stays valid across query changes and the enhancer effect below
  // does NOT tear down and re-apply every behavior on the page when an overlay
  // opens. `usePage().route.query` is therefore current whenever it is READ,
  // but reading it does not subscribe — for a re-render use `useRoute().query`
  // or `useSearchParam`.
  const route = useMemo<RouteInfo>(() => toInfo(entry), [entry])
